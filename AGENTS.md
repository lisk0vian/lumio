...
- React exists only as `*.island.tsx` hydration roots under `src/features/<domain>/` (calculator, consumption, history, settings, share, tips, glossary, shell) — sole exception: `history-rail.tsx` + `history-rail.loader.ts`, mounted on demand when the first record exists because Astro has no `client:hover` (deferred store rehydration via `useStoreRehydration`). Islands receive `lang: AppLang` and use `useTranslations(lang)`; never `t()`/`useActiveLang` (deleted) and never store language (store v2 migration drops it).
